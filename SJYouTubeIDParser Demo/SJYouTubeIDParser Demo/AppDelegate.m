  ...
  [self.circularProgressIndicator startAnimation:nil];
    ...
    [youtubeParser getVideoInfoFromURL:[self.textField stringValue] completionHandler:^(NSString *videoID, NSString *videoTitle, NSURL *thumbnailURL, NSString *videoDescription, NSString *viewCount, long long likeCount, long long dislikeCount, NSDate *uploaded, NSString *uploader, NSError *error) {
        ...
        if (error == nil) {
            ...
            [self.viewCountTextField setStringValue:[NSString stringWithFormat:@"%@ views",[formatter stringFromNumber:[NSNumber numberWithInteger:[viewCount integerValue]]]]];
            
            //Likes
            
            [self.likesLabel setStringValue:[NSString stringWithFormat:@"%@ likes",[formatter stringFromNumber:[NSNumber numberWithLongLong:likeCount]]]];

             
             //Dislikes
             [self.dislikesLabel setStringValue:[NSString stringWithFormat:@"%@ dislikes",[formatter stringFromNumber:[NSNumber numberWithLongLong:dislikeCount]]]];
            ...
            if ([error code] == 10) {
                ...
                NSLog(@"Video is deleted");
                [NSApp presentError:error];
                
                
            } else if ([error code] == 30){
                NSLog(@"Video is private");
                [NSApp presentError:error];
